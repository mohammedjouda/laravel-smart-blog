...
<head>
    ...
    <title>Create Post | Paperink</title>
...
<body class="bg-background text-on-surface font-body-md min-h-screen">
    <header class="sticky top-0 z-50 bg-background border-b border-outline-variant/30">
        <nav class="flex justify-between items-center w-full px-margin-desktop py-4 max-w-container-max mx-auto">
            ...
            <div class="flex items-center gap-4">
                ...
                <button form="post-form" class="px-6 py-2 rounded-lg bg-primary-container text-on-primary font-label-md hover:bg-primary transition-all" type="submit">Publish</button>
            </div>
        </nav>
    </header>
    ...
    <main class="max-w-container-max mx-auto px-margin-desktop py-12">
        <div class="mb-10">
            <p class="font-label-md text-label-md text-primary uppercase tracking-widest">New entry</p>
            <h1 class="font-headline-lg text-headline-lg mt-2">Create Post</h1>
        </div>

        <form id="post-form" action="{{ route('dashboard.posts.store') }}" method="POST" enctype="multipart/form-data">
            
            @include('dashboard.posts._form', ['submitLabel' => 'Create Post'])
        </form>
    </main>
</body>
